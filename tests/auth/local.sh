...
if [[ "$case_name" == wrong-token ]]; then token="${token%?}A"; fi
...
expected=auth.pong
case "$case_name" in
  wrong-token) expected=auth.invalid_credential ;;
  ...
  malformed-frame) expected=protocol.malformed ;;
  unsupported-version) expected=protocol.unsupported_version ;;
  oversized-frame) expected=protocol.frame_too_large ;;
  connection-limit) expected=limit.auth_connections ;;
  request-limit) expected=limit.auth_requests ;;
  session-limit) expected=limit.auth_sessions ;;
...
if [[ "$case_name" == connection-limit || "$case_name" == request-limit || "$case_name" == session-limit ]]; then
  grep -q "\"code\":\"$expected\"" "$log" || { echo "limit case expected $expected" >&2; cat "$log" >&2; exit 1; }
elif [[ "$case_name" == malformed-frame || "$case_name" == unsupported-version || "$case_name" == oversized-frame ]]; then
  grep -q '"code":"auth.fault_applied"' "$log" || { echo "auth fault was not observed" >&2; exit 1; }
  grep -q "\"code\":\"$expected\"" "$log" || { echo "fault case expected $expected" >&2; cat "$log" >&2; exit 1; }
elif [[ "$case_name" != exchange-restart ]]; then
  grep -q "\"code\":\"$expected\"" "$log" || { echo "expected $expected" >&2; cat "$log" >&2; exit 1; }
fi
if [[ -n "$companion" && "$case_name" != exchange-restart ]]; then
...
if [[ "$case_name" == exchange-restart ]]; then
  for endpoint_log in "$log" "$out/server.ndjson"; do
    for _ in $(seq 1 200); do grep -q '"event":"auth_readiness","ready":true' "$endpoint_log" && break; sleep .05; done
    grep -q '"event":"auth_readiness","ready":true' "$endpoint_log" || { echo "initial readiness missing: $endpoint_log" >&2; exit 1; }
  done
...
PY
  ...
  initial_server_peer=$(python3 - "$out/server.ndjson" <<'PY'
import json, sys
for line in open(sys.argv[1]):
    row = json.loads(line)
    if row.get('event') == 'started': print(row['peer_id']); break
PY
  ...
  for endpoint_log in "$log" "$out/server.ndjson"; do
    for _ in $(seq 1 300); do grep -q '"event":"auth_readiness","ready":true,"generation":2' "$endpoint_log" && break; sleep .05; done
    grep -q '"event":"auth_readiness","ready":false' "$endpoint_log" || { echo "restart lacked readiness loss: $endpoint_log" >&2; exit 1; }
    grep -q '"event":"auth_readiness","ready":true,"generation":2' "$endpoint_log" || { echo "restart lacked readiness recovery: $endpoint_log" >&2; exit 1; }
  done
  kill -INT "$component_pid" "$companion_pid" 2>/dev/null || true
  for endpoint_log in "$log" "$out/server.ndjson"; do
    for _ in $(seq 1 100); do grep -q '"event":"terminal"' "$endpoint_log" && break; sleep .05; done
    [[ "$(grep -c '"event":"terminal"' "$endpoint_log")" -eq 1 ]] || { echo "restart endpoint terminal cardinality mismatch: $endpoint_log" >&2; exit 1; }
  done
  grep -q "\"peer_id\":\"$initial_peer\"" "$log" || { echo "restart changed peer identity" >&2; exit 1; }
  grep -q "\"peer_id\":\"$initial_server_peer\"" "$out/server.ndjson" || { echo "restart changed server identity" >&2; exit 1; }
fi
! grep -E "$client_token|$server_token|$rotation_token|$client_digest|$server_digest|$rotation_digest|token_secret|raw_ticket|$exchange_key|$client_key|$server_key" "$out"/* >/dev/null 2>&1 || { echo "secret leaked" >&2; exit 1; }
if [[ "$case_name" == exchange-restart ]]; then
  exit 0
