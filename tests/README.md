...
`tests/registry/local.sh --case <name>` is the single registry harness entry point; it prepares a run-scoped artifact directory, returns 2 for invalid CLI usage, and exits nonzero when a prerequisite or observed assertion fails. `tests/resolution/local.sh --case <name|all>` is the single Plan 05a entry point; it prepares run-scoped identities, credentials, ticket/signing files, routes, artifacts, and privacy cleanup. It executes the existing TCP/QUIC and rejection cases plus idempotency, direct fallback, replay, the complete same-process binding matrix, expiry, reuse, and 64-open evidence-backed cases. The remaining named cases fail explicitly until their required orchestration evidence exists; Ping, a case label, synthetic response, or server-only event is never proxy evidence. See [`resolution/README.md`](resolution/README.md) for the exact case matrix and owner-executed checks.
