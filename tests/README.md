...
`tests/registry/local.sh --case <name>` is the single registry harness entry point; it prepares a run-scoped artifact directory, returns 2 for invalid CLI usage, and exits nonzero when a prerequisite or observed assertion fails. `tests/tunnel/local.sh --case <name|all>` is the single Plan 06 raw TCP entry point. It builds the binaries, creates run-scoped fixtures below `target/`, runs real exchange/client/server processes, validates endpoint bytes or stable upstream failure/idle codes, and scans lifecycle output for private data. Direct/relay topology loss, soak, RSS, and platform evidence remain explicitly owner-executed. `tests/resolution/local.sh --case <name|all>` is the single Plan 05a entry point; it prepares run-scoped identities, credentials, ticket/signing files, routes, artifacts, and privacy cleanup. It executes all 20 declared TCP/QUIC, rejection, idempotency, fallback, replay, binding, expiry, reuse, concurrency, limit, replacement, restart, and graceful-drain cases through real product binaries. Ping, a case label, synthetic response, or server-only event is never proxy evidence. See [`resolution/README.md`](resolution/README.md) for the exact case matrix and owner-executed checks.
