...
`netns.sh` requires Linux, `iproute2`, and the privileges needed for namespace/network mutation. C14 is manual and is documented in [`two-host.md`](two-host.md). The live relay reservation, probe payload, and matrix execution remain outstanding, so no C01–C14 result is currently claimed. Dependency verification is provisioned with `cargo-deny 0.20.2`; run `cargo deny check` from the repository root.
