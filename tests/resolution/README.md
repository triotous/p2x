...
- `concurrent-opens`: 128 independently correlated resolve, exact-open, ticket, and stream results; the client emits an authoritative `route_owner_high_water` event and the runner requires the exact bounded 64-open owner peak rather than relying on periodic sampling.
...
Each successful summary has `case`, `passed`, and `observed_assertions` derived from executed lifecycle/endpoint checks. The assertions include client terminal, client↔exchange resolution correlation, client↔server authorization correlation where applicable, exact selected connection where applicable, and clean privacy scan. Case-specific checks add fingerprint/issuance cardinality, path order, replay rejection, expiry rejection, stream uniqueness, or the authoritative 128-total/64-concurrent owner boundary.
