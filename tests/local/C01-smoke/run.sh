...
for _ in $(seq 1 200); do
  if grep -q '"event":"probe_completed"' "$OUT/client.ndjson" && grep -q '"event":"probe_completed"' "$OUT/server.ndjson"; then
    [[ $(grep -c '"event":"terminal"' "$OUT/client.ndjson") -eq 1 ]] || break
    [[ $(grep -c '"event":"probe_completed"' "$OUT/server.ndjson") -eq 1 ]] || break
  ...
  if grep -q '"event":"terminal".*"result":"failed"' "$OUT/client.ndjson"; then break; fi
  sleep 0.1
done
