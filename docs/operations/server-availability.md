...
On graceful shutdown, proxy admission first rejects new Opens, readiness becomes false, one current registration is withdrawn with a five-second bound, Phase 3 handshake workers drain, the circuit listener and exchange connections close, and timers/resources drain. A ticket whose registration or server authorization revision is stale fails closed even if its signature remains valid. Exchange shutdown rejects new work, drains owned responses for up to five seconds, then clears registry and admission state. The registry is in-memory and single-exchange in this phase; exchange restart empties it.
