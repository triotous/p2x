...
Product-mode failures are isolated to the affected local ingress: resolve, path, proxy, connection, deadline, capacity, and pre-Accept EOF/I/O failures reject and release only that ingress while listeners and unrelated tunnels continue. Pre-Accept local EOF/error emits one cancellation event immediately. Per-server capacity counts active plus pending setup owners literally against `max_streams_per_server`; pending and active transitions are checked.
