...
The stream carries opaque bytes only. It has independent directions, propagates half-close, applies backpressure per direction, and uses the server service idle timeout. HTTP Host and TLS SNI adapters are documented in [`domain-ingress.md`](domain-ingress.md); all listener kinds share the configured ingress connection budget. A P2P connection loss resets the active stream; a later local connection resolves and selects a new path. Exchange control loss does not interrupt an already accepted stream.

`upstream.connect_timeout` and `upstream.connect_failed` occur before Accepted and consume the ticket. Server Open verification, owner decision, upstream dial, and Accepted write share one cancellable five-second worker deadline; configured connect timeout is clipped to the remaining budget. `upstream.idle_timeout` is a post-Accepted terminal. There is no transparent retry or migration of an active TCP stream. Accepted tunnel records include component side, selected path, terminal class, setup duration, correlation hashes, EOF state, and directional byte counters. The executable local gate is `./tests/tunnel/local.sh --case <name|all>`. Its 33 names cover direct/relay byte flow, half-close, 256 MiB backpressure, per-ingress recovery, client/server N+1 limits, setup/active shutdown, deadline stages, idle isolation, terminal correlation, 64 sustained streams, actual 128-stream headroom, and before/peak/after resource evidence. Each run requires exact generated-value privacy scans, one terminal per process, one tunnel terminal per Accepted stream on each side, and final zero logical resources. Linux namespace/firewall, cross-host, platform, and long-running soak evidence remain separate owner-executed checks.
