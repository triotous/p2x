...
The exchange scopes registry state to the authenticated transport peer and current session. Phase 3 product servers advertise DCUtR as an accepted capability; exact service lookup for `/p2x/resolve/1` returns an owned matched-service view without exposing registry enumeration. A server must have role `Server`, `register_services`, the supported `standard` quota profile, current auth, and an accepted relay reservation. `RELAY_V2` plus at least one direct transport capability is required; `DCUTR` is rejected by the product registry path.
