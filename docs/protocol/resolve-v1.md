...
The locally verified resolution cases are listed in [`tests/resolution/README.md`](../../tests/resolution/README.md). The runner executes and asserts registration replacement, low-limit saturation, restart recovery, graceful drain, and the complete binding matrix; only the deployment/platform checks listed in the test runbook remain owner-executed.
